...
if ! "$interprete" -c 'import flask' >/dev/null 2>&1; then
  ...
  echo ".venv/bin/python -m pip install -r unidad-01-introduccion-ia/practicas/practica-06-sistema-experto-salud/requirements.txt"
  exit 1
fi
exec "$interprete" "$directorio/06_sistema_experto_salud.py"
