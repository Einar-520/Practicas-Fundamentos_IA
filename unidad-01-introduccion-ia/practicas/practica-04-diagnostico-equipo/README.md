...
```bash
python3 -m venv .venv
.venv/bin/python -m pip install -r unidad-01-introduccion-ia/practicas/practica-04-diagnostico-equipo/requirements.txt
```
...
Abre http://localhost:5104 en tu navegador. Mantén la terminal abierta;
Ctrl+C detiene el servidor. También puedes ejecutar `bash iniciar_practica_web.sh 4`
...
| Archivo | Función |
| --- | --- |
| `04_diagnostico_equipo.py` | Reglas de la práctica, validación y servidor Flask. |
...
ejecuta `04_diagnostico_equipo.py` con Python desde cualquier directorio.
