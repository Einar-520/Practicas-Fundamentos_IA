...
```bash
python3 -m venv .venv
.venv/bin/python -m pip install -r unidad-01-introduccion-ia/practicas/practica-02-sistema-examen/requirements.txt
```
...
Abre http://localhost:5102 en tu navegador. Mantén la terminal abierta;
Ctrl+C detiene el servidor. También puedes ejecutar `bash iniciar_practica_web.sh 2`
...
| Archivo | Función |
| --- | --- |
| `02_sistema_examen.py` | Reglas de la práctica, validación y servidor Flask. |
...
ejecuta `02_sistema_examen.py` con Python desde cualquier directorio.
