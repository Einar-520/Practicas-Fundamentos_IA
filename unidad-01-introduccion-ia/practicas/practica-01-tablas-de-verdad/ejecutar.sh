...
if ! "$interprete" -c 'import flask' >/dev/null 2>&1; then
  ...
  echo ".venv/bin/python -m pip install -r unidad-01-introduccion-ia/practicas/practica-01-tablas-de-verdad/requirements.txt"
  exit 1
fi
exec "$interprete" "$directorio/01_tablas_de_verdad.py"
