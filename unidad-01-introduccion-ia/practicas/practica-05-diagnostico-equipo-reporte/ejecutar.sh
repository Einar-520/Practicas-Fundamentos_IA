...
if ! "$interprete" -c 'import flask' >/dev/null 2>&1; then
  ...
  echo ".venv/bin/python -m pip install -r unidad-01-introduccion-ia/practicas/practica-05-diagnostico-equipo-reporte/requirements.txt"
  exit 1
fi
exec "$interprete" "$directorio/05_diagnostico_equipo_reporte.py"
