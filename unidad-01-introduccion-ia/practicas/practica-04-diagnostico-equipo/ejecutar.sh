...
if ! "$interprete" -c 'import flask' >/dev/null 2>&1; then
  ...
  echo ".venv/bin/python -m pip install -r unidad-01-introduccion-ia/practicas/practica-04-diagnostico-equipo/requirements.txt"
  exit 1
fi
exec "$interprete" "$directorio/04_diagnostico_equipo.py"
